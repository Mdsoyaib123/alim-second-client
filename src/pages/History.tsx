...
const History = () => {
  ...
  const singleWithdrawUserId = singleWithdrawId
    ? parseInt(singleWithdrawId, 10)
    : 0;
        ...
        <div className="bg-white rounded-2xl border border-amber-100/80 shadow-2xs overflow-hidden">
          ...
          {!isLoading && !error && !hasData && (
            <div className="p-12 text-center text-slate-500">
              ...
              <p className="font-serif font-bold text-base text-text-dark">
                No transactions found
              </p>
              <p className="text-xs text-slate-400 mt-1">
                You haven't made any{" "}
                {activeTab === "withdraw" ? "Cashout" : subTab} transactions yet
              </p>
            </div>
          )}
            ...
            withdrawData.data.length > 0 && (
              <div className="divide-y divide-amber-100/70">
                {withdrawData.data.map((item) => (
                  <div
                    key={item._id}
                    className="p-4 hover:bg-amber-50/40 transition-colors"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-serif font-bold text-sm text-text-dark">
                            Withdrawal
                          </p>
                          <p className="text-[11px] text-slate-400 font-light">
                ...
                {otherData.data.map((item: HistoryItem) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p
                          className={`text-sm font-serif font-bold ${getHistoryColor(item.historyType)}`}
                        >
