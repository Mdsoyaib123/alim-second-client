...
const CashOut = () => {
  ...
  const hasWithdrawalAddress =
    user.withdrawalAddressAndMethod !== null &&
    user.withdrawalAddressAndMethod !== undefined;
          ...
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  className={`${isMobileBanking && ["bkash", "nagad", "rocket", "upay"].includes(user.withdrawalAddressAndMethod?.mobileBankingName?.toLowerCase() || "") ? "" : "bg-yellow-100 p-2 rounded"}`}
                >
                  {isMobileBanking ? (
                    (() => {
                      const provider =
                        user.withdrawalAddressAndMethod?.mobileBankingName?.toLowerCase();
                      switch (provider) {
                        case "bkash":
                          return (
                            <img
      ...
      <Card className="bg-slate-50 border-slate-200">
        <CardContent className="pt-1">
          <p className="text-xs font-medium text-slate-800 mb-2">
            Rules : Minimum Cash Out ৳500
          </p>
          <p className="text-xs font-medium text-slate-700 mb-1">Tips:</p>
          <p className="text-xs text-slate-600 leading-relaxed">
            Please bind in the correct bank account information. If the Cash Out
            is successful but the bank account details are wrong, platform will
            not be responsible. Therefore, please double-check your bank account
            information.
          </p>
        </CardContent>
      </Card>
