...
const SignInBonus: React.FC = () => {
  return (
    <section className="w-full py-6 px-4">
      <div className="max-w-4xl mx-auto bg-[#faf5ed] border border-amber-200/60 rounded-md p-5 sm:p-6 shadow-xs">
        {/* Header */}
        ...
        <div className="space-y-2">
          {BONUS_DAYS.map((item, idx) => (
            <div
              key={idx}
              className="flex items-center justify-between bg-white/90 rounded-sm p-3 px-4 border border-amber-100 shadow-2xs hover:border-amber-300 transition-colors"
            >
